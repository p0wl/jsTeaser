...
var jsTeaser = (function () {
    ...
    that.summarize = function (title, text) {
        ...
        summaries = that.top(ranks);

        return summaries;
    };

    that.score = function (sentences, titleWords, keywords) {
        ...
        for (var i = 0; i < sentences.length; i++) {
            var s = sentences[i];
            result = that.scoreSentence(s, titleWords, keywords, sentencePosition(i + 1, senSize));
            ranks.push(result);
        }
        return ranks;
    };

    that.scoreSentence = function (s, titleWords, keywords, sentencePositionVar) {
        ...
        var score = (titleFeature * 1.5 + frequency * 2.0 + sentenceLength * 1.0 + sentencePositionVar * 1.0) / 4.0;
        return {sentence: s, score: score};
    };

    that.top = function (ranks) {
